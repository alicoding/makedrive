...
function handleRequest(syncManager, data) {
  ...
  if(data.is.chksum && session.is.ready &&
    ...
    onError(syncManager, new Error(data.conent));
  }
}
...
// HUMPH: I need to confirm this is right, but without it I don't get a completed event
    sync.onCompleted();
  }
...
function handleMessage(syncManager, data, flags) {
  try {

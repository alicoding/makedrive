...
rsync.patch = function(fs, path, diff, options, callback) {
  ...
  function syncEach(entry, callback) {
    ...
    if(entry.contents) {
      return rsync.patch(fs, Path.join(path, entry.path), entry.contents, options, function(err) {
        if(err) {console.log("line number: 679", err)
          return callback(err);
        }
    ...
    // Link
    else if (entry.link) {console.log("line number: 687", err)
      return fs.symlink(entry.link, syncPath, function(err){
        if(err) {console.log("line number: 689", err)
          return callback(err);
        }
    ...
    fs.readFile(syncPath, function(err, data) {
      ...
      if(err) {
        if(err.code !== 'ENOENT') {console.log("line number: 713", err)
          return callback(err);
        }
        raw = new Buffer(0);
      } else {
      ...
      console.log("line number: 739", syncPath, buf)
      fs.writeFile(syncPath, buf, function(err) {
        if(err) {console.log("line number: 740", err)
          return callback(err);
        }

        if(!options.time) {console.log("line number: 744", 'here')
          return callback();
        }

        // Updates the modified time of the node
        fs.utimes(syncPath, entry.modified, entry.modified, function(err) {
          if(err) {console.log("line number: 749", err, syncPath)
            return callback(err);
          }
  ...
  if(diff) {
    async.each(diff, syncEach, function(err) {console.log(err)
